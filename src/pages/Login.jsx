...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    axios({
      method: 'POST',
      url: 'https://after-eleven-server.herokuapp.com/api/login',
      data: body,
    })
  ...
  return (
    <section className="xl:max-w-[1280px] w-full mx-auto sm:px-10 px-20 flex flex-col sm:mt-40 mt-28 h-[70vh]">
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-sm mx-auto my-auto">
        ...
        {error && (
          <div className="flex justify-center items-center gap-2 text-semibold text-lg text-red-500 m-5">{error}</div>
        )}
      </form>
    </section>
  );
