...
const AdminMenu = () => {
  const { data: categories, isLoading } = useGetData(true, categoriesAPI);
  ...
  useEffect(() => {
    if (sortValue === 'asc') {
      axios.get(menuAscSortPrice).then(function (response) {
        setItemsList(response.data).catch((err) => console.log(err));
      });
    }
    if (sortValue === 'desc') {
      axios.get(menuDescSortPrice).then(function (response) {
        setItemsList(response.data).catch((err) => console.log(err));
      });
    }
    if (sortValue === 'all') {
      axios.get(menuAPI).then(function (response) {
        setItemsList(response.data).catch((err) => console.log(err));
      });
    }
  }, [sortValue]);
  return (
    <section className="xl:max-w-[1280px] w-full mx-auto sm:px-10 px-20 flex flex-col sm:mt-40 mt-28">
      ...
      <Link to="/admin-menu/addNewItem" className="btn-main sm:text-base text-[12px] mx-auto mb-10">
        Add New Food
      </Link>
      ...
      <div className="flex flex-wrap xs:gap-8 gap-2 mb-10 xs:items-center xs:justify-center justify-start">
        {categories.map((category) => {
          return (
            <button className="btn-sub capitalize" key={category.id} onClick={() => setIdItemsDisplay(category.id)}>
              {category.name}
            </button>
          );
        })}
        <select
          name="sortPrice"
          className="border-dimWhite border-solid border-2 bg-slate-400 text-black py-2 px-5 rounded-md"
      ...
      <div className="flex flex-wrap justify-center items-center gap-5">
        {!isLoading &&
          (idItemsDisplay === 0
            ...
            : itemsList
                .filter((item) => item.categoryId === idItemsDisplay)
                .map((item) => {
                  return (
                    <CardItem
                      key={item.id}
