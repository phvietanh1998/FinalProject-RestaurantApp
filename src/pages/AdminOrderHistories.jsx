    ...
    <section className="xl:max-w-[1280px] w-full mx-auto sm:px-10 px-20 flex flex-col sm:mt-40 mt-28">
      ...
      <div className="text-dimWhite text-lg flex flex-col items-start gap-5">
        ...
        <div className="flex items-center gap-5 mb-10">
          <label htmlFor="byYear" className="">
            Filter by Amount
          </label>
          <select
            name="byYear"
            className="bg-gray-300 text-black px-3 py-1 rounded-md text-center"
        ...
        {isLoading
          ? orders.map((order) => {
              return (
                <div
                  key={order.id}
                  className="w-[420px] sm:h-[520px] h-fit border-solid border-2 border-primary text-lg text-dimWhite rounded-md"
                >
                  <div className="flex justify-between items-center gap-2 p-5">
                    <div className="flex flex-col gap-3 justify-center">
                      ...
                      <div>
                        Customer name: <span className="font-semibold capitalize">{order.name}</span>
                      </div>
                      <div>
                        Username: <span className="font-semibold">{order.username}</span>
                      </div>
                      <div>
                        Phone number: <span className="font-semibold">{order.phone}</span>
                      </div>
                      <div>
                        Address: <span className="font-semibold capitalize">{order.address}</span>
                      </div>
                      <div>
                        Time: <span className="font-semibold capitalize">{order.time}</span>
                      </div>
                      <div>
                        Amount: <span className="font-semibold capitalize">{order.amount}</span>
                      </div>
                      <div>
                        Bill's Info:
                        <p className="italic text-xl border-l-4 border-solid pl-5 p-2 mt-3 font-semibold">
      ...
      {isShow && (
        <div className="flex gap-20 items-center justify-center mt-10">
          <button className="btn-sub flex justify-center items-center gap-5 font-semibold text-xl" onClick={prevPage}>
            <AiOutlineArrowLeft /> Prev{' '}
          </button>
          <button className="btn-sub flex justify-center items-center gap-5 font-semibold text-xl" onClick={nextPage}>
