...
import { isLoginedSelector, accountInfoSelector, getRoleSelector } from '../redux/selectors';
    ...
    <header className=" bg-black fixed top-0 w-full shadow-2xl shadow-primary z-[5]">
      <div className="xl:max-w-[1280px] w-full mx-auto sm:px-10 px-5">
        <div className="flex items-center justify-items-center text-primary justify-between">
          ...
          {!isLogined || accountInfo.role === 'user' ? (
            pathname === '/' ? (
          ...
          <div className="flex md:justify-start items-center gap-5">
            {isLogined ? (
              <div className="text-lg">
                Welcome,{' '}
                <Link to="/information" className="hover:font-semibold transition-all">
                  {accountInfo.username}
                </Link>{' '}
              </div>
            ) : null}
            {isLogined ? (
              <Link to="/" className="sm:text-lg text[12px]" onClick={handleIsLogout}>
                Logout
              </Link>
            ) : (
              <Link to="/login" className="sm:text-lg text[12px]">
                Login
              </Link>
            )}
          ...
          <nav
            className={`${
              toggle ? 'sm:flex' : 'hidden'
            } p-6 bg-black-gradient absolute top-40 right-0 min-w-[140px] rounded-xl sidebar`}
          >
            {!isLogined || accountInfo.role === 'user' ? (
              pathname === '/' ? (
                <ul className="hidden sm:flex justify-center list-none sm:text-2xl text-xl col-span-10 gap-14">
              ...
              <ul className="list-none flex flex-col justify-end items-center flex-1 gap-5">
                ...
                <li>
                  <Link href="/admin-feedback">Feedbacks</Link>
                </li>
                <li>
