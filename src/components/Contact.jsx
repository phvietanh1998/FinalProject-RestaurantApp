...
import { BsGithub, BsInstagram, BsCheckCircle } from 'react-icons/bs';
...
const Contact = () => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm('service_o2p3vr8', 'template_byhludd', form.current, '-X130pjpLmTnZDT9H').then(
      (result) => {
        ...
        <div className="sm:grid grid-cols-2 gap-10 mt-10 flex flex-col">
          <form ref={form} onSubmit={sendEmail} className="flex flex-col justify-center items-center gap-5">
            ...
            <textarea
              name="message"
              rows="10"
              placeholder="Say something with us..."
              className="input-default text-lg xs:resize-none"
            ></textarea>
